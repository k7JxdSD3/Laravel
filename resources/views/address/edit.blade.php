...
<div class="panel-heading"><h3>住所編集</h3></div>

<div class="panel-body">
<form class="form-horizontal" method="POST" action="{{ route('address.edit', ['address_id' => $address->id]) }}">
{{ csrf_field() }}
...
<div class="col-md-6">
<input id="name" type="text" class="form-control" name="name" value="{{ $address->name }}" required autofocus>
...
半角数字7桁(ハイフン無し)
<div class="col-md-6">
<input id="zip" type="number" class="form-control" name="zip" onKeyUp="AjaxZip3.zip2addr(this, '', 'prefectures', 'city');" value="{{ $address->zip }}" required autofocus>
...
<option value="{{ $address->prefectures }}" selected>{{ $address->prefectures }}</option>
...
半角数字(ハイフン無し)
<div class="col-md-6">
<input id="phone_number" type="number" class="form-control" name="phone_number" value="{{ $address->phone_number }}" required autofocus>
