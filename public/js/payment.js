...
/* Stripe Elementsを使ったFormの各パーツをどんなデザインにしたいか定義 */
const style = {
  base: {
    fontSize: '12px',
    color: "#32325d",
...
/* フォームでdivタグになっている部分をStripe Elementを使ってフォームに変換 */
...
/*  何も処理をかまさないとそのままクレジットカード情報が送信されてしまうので
 *  一旦HTMLのFormタグが従来持っている送信機能を停止させる
 */
  e.preventDefault();

/* Stripe.jsを使って、フォームに入力されたコードをStripe側に送信、
 * 今回紹介している方法の場合、「カード名義」だけはStripe Elementsの仕組みを使っていないため
...
/* errorが返ってきた場合はその旨を表示 */
    if (result.error) {
      alert("カード登録処理時にエラーが発生しました。カード番号が正しいことをもう一度ご確認いただくか、別のクレジットカードを登録してみてください。");
    } else {

/* 暗号化されたコードが返ってきた場合は以下のStripeTokenHandler関数を実行、
 * その際に引数として暗号化されたコードを渡してあげる
 */
 ...
 * 値にSrtipeから返ってきた暗号情報を設定
 * そして、実際にフォームのないようを送信（事実上、送信されるのは暗号化情報のみとなる）
 */
  function stripeTokenHandler(token) {
    const form = document.getElementById('form_payment');
    const hiddenInput = document.createElement('input');
    hiddenInput.setAttribute('type', 'hedden');
